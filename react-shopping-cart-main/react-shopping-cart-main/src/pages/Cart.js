...
const Cart = () => {
  ...
  useEffect(() => {
    ...
    //   fetch("https://order-pizza-api.herokuapp.com/api/swagger.json", {
    //     method: "POST",
    //     headers: {
    ...
    //   .then((res) => res.json())
    //   .then((products) => {
    //     //console.log("response checking" + JSON.stringify(products));
    //     // let dummydata = JSON.stringify(products);
    //     //let objData = Object.values(dummydata);
    //     console.log("jasjf", products.products);
    //     // console.log("Dummy", dummydata['products']);
    ...
    fetch("https://star-spark-pasta.glitch.me/api/products", {
      ...
      .then((res) => res.json())
      .then((products) => {
        console.log("response checking", products);
        setProducts(products);
        togglePriceFetched(true);
      });
  }, [cart, priceFetched]);
          ...
          return (
            <li className="mb-12" key={product._id}>
              <div className="flex items-center justify-between">
                ...
                <span>₹ {getSum(product._id, parseInt(product.price))}</span>
                <button
                  onClick={() => {
